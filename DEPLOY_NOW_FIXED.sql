...
BEGIN
  ...
  WITH all_results AS (
    ...
    WHERE m.embedding IS NOT NULL
      AND (
        -- Handle both uuid and text user_id types
        (pg_typeof(m.user_id) = 'uuid'::regtype AND m.user_id::text = p_user_id) OR
        (pg_typeof(m.user_id) = 'text'::regtype AND m.user_id = p_user_id)
      )
    ...
    WHERE f.embedding IS NOT NULL
      AND (
        (pg_typeof(f.user_id) = 'uuid'::regtype AND f.user_id::text = p_user_id) OR
        (pg_typeof(f.user_id) = 'text'::regtype AND f.user_id = p_user_id)
      )
    ...
    WHERE t.embedding IS NOT NULL
      AND (
        (pg_typeof(t.user_id) = 'uuid'::regtype AND t.user_id::text = p_user_id) OR
        (pg_typeof(t.user_id) = 'text'::regtype AND t.user_id = p_user_id)
      )
    ...
    WHERE kb.embedding IS NOT NULL
      AND (
        (pg_typeof(kb.user_id) = 'uuid'::regtype AND kb.user_id::text = p_user_id) OR
        (pg_typeof(kb.user_id) = 'text'::regtype AND kb.user_id = p_user_id)
      )
...
BEGIN
  ...
  FROM messages
  WHERE (
    (pg_typeof(user_id) = 'uuid'::regtype AND user_id::text = p_user_id) OR
    (pg_typeof(user_id) = 'text'::regtype AND user_id = p_user_id)
  )
