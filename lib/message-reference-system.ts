  ...
  async storeMessage(
    ...
    try {
      ...
      await ConversationLogger.logSystemEvent('MESSAGE_STORED', {
        ...
        has_code: extractedData.code_blocks?.length > 0,
        has_decisions: extractedData.decisions?.length > 0,
        has_action_items: extractedData.action_items?.length > 0
      });
