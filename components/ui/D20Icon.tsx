 ...
 * D20 Icon Component
 *
 * Geometrically correct icosahedron (20-sided die) with TRUE 3D rotation
 * Based on golden ratio mathematics with transparent wireframe
 */
...
const sizeMap = {
  sm: 108,  // 54 * 2 (100% increase)
  md: 180,  // 90 * 2 (100% increase)
  lg: 288,  // 144 * 2 (100% increase)
  xl: 432,  // 216 * 2 (100% increase)
};
...
export default function D20Icon({
  ...
  if (!isMounted) {
    ...
    return (
      <div style={{ width: pixelSize, height: pixelSize }} className={`inline-block ${className}`} {...props}>
        {renderD20()}
      </div>
    );
  }
  ...
  return (
    <div
      className={`inline-block ${glow ? 'pulse-glow' : ''} ${className}`}
      style={{
        width: pixelSize,
        height: pixelSize
      }}
