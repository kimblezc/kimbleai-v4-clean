 ...
 *
 * Usage: npx ts-node scripts/validate-env-whitespace.ts
 */
...
interface ValidationIssue {
  type: 'actual_whitespace' | 'literal_whitespace' | 'format' | 'missing' | 'length';
 ...
 */
function detectActualWhitespace(value: string): ValidationIssue[] {
 ...
 */
function detectLiteralWhitespace(value: string): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
 ...
 */
function validateApiKeyFormat(varName: string, value: string): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
 ...
 */
function validateVariable(variable: EnvVariable): ValidationIssue[] {
  ...
  allIssues.forEach(issue => formatIssues.push(issue));
 ...
 */
function maskValue(value: string, varName: string): string {
  // Don't mask URLs
