 ...
 * Run with: node scripts/debug-archie-dashboard.mjs
 */
  ...
  if (tasksError) {
    ...
    if (allTasks && allTasks.length > 0) {
      ...
      allTasks.slice(0, 5).forEach((task, i) => {
        ...
        console.log(`     Description: ${task.description?.substring(0, 100) || 'N/A'}...`);
        console.log(`     Priority: P${task.priority} | Created: ${new Date(task.created_at).toLocaleString()}`);
        console.log('');
      });
    }
  }
  ...
  if (findingsError) {
    ...
    if (allFindings && allFindings.length > 0) {
      ...
      allFindings.slice(0, 10).forEach((finding, i) => {
        console.log(`  ${i + 1}. [${finding.finding_type}/${finding.severity}] ${finding.title}`);
        console.log(`     "${finding.description?.substring(0, 100) || 'N/A'}..."`);
