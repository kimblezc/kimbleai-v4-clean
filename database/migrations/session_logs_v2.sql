-- Session Logs System v2
-- Auto-detects users.id type and creates matching foreign key
...
BEGIN
  ...
  IF user_id_type = 'uuid' THEN
    ...
    RAISE NOTICE 'Created session_logs with user_id UUID type';

  ELSE
    -- users.id is TEXT or other type
    CREATE TABLE IF NOT EXISTS session_logs (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    RAISE NOTICE 'Created session_logs with user_id TEXT type';
  END IF;
END $$;
...
DROP TRIGGER IF EXISTS trigger_update_session_logs_search_vector ON session_logs;
...
DROP TRIGGER IF EXISTS trigger_update_session_logs_timestamp ON session_logs;
...
DROP POLICY IF EXISTS session_logs_insert_own ON session_logs;
...
DROP POLICY IF EXISTS session_logs_update_own ON session_logs;
...
DROP POLICY IF EXISTS session_logs_delete_own ON session_logs;
...
BEGIN
  SELECT data_type INTO user_id_col_type
  FROM information_schema.columns
  WHERE table_name = 'session_logs'
    AND column_name = 'user_id';

  RAISE NOTICE '✅ session_logs table created successfully';
  RAISE NOTICE '   user_id type: %', user_id_col_type;
END $$;
