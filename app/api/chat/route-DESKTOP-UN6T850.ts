...
// Legacy OpenAI client for embedding functions (will be migrated)
...
async function generateEmbedding(text: string, userId: string = 'zach-admin-001'): Promise<number[] | null> {
  try {
    const costMonitoredOpenAI = getCostMonitoredOpenAI(userId);
    const response = await costMonitoredOpenAI.embeddings({
...
- For Zach (admin): Provide system insights and admin-level information when relevant`
    ...
    // Get cost-monitored OpenAI client for this user
    const costMonitoredOpenAI = getCostMonitoredOpenAI(userData.id);

    // Get AI response with cost monitoring and throttling
    const completion = await costMonitoredOpenAI.chatCompletions(modelParams);
    ...
    // Save user message
    const userEmbedding = await generateEmbedding(userMessage, userData.id);
    ...
    // Save AI response
    const aiEmbedding = await generateEmbedding(aiResponse, userData.id);
    ...
    if (facts.length > 0) {
      for (const fact of facts) {
        const factEmbedding = await generateEmbedding(fact.content, userData.id);
