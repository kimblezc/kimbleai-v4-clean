...
- For Zach (admin): Provide system insights and admin-level information when relevant`;
    ...
    // Define function tools with examples (Phase 1: Advanced Tool Use)
    // Includes: Gmail, Drive, Files, Calendar, RAG Search, Knowledge Graph, Advanced AI
    const baseTools = getToolsForAPI(); // Get enhanced tools with examples (internal only)
    const advancedTools = getAdvancedToolsForAPI(); // RAG, Knowledge Graph, Perplexity, DeepSeek, ElevenLabs
    const tools = [...baseTools, ...advancedTools]; // Combined tool registry (20+ tools)

    console.log(`[TOOLS] Loaded ${tools.length} enhanced tools with usage examples`);
