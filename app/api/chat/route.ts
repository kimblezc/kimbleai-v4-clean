...
/**
 * Execute Agent Mode - Route queries to specialized agents
 */
async function executeAgentMode(agentId: string, messages: any[], userData: any, conversationId: string) {
  try {
    ...
    console.log(`[AgentMode] Executing agent: ${agentId} for user: ${userData.name}`);

    let agentResponse = '';
    let metadata: any = { agent: agentId };

    switch (agentId) {
      case 'drive-intelligence':
        // Search Drive with enhanced analysis
        agentResponse = `## 📁 Drive Intelligence Analysis\n\n`;
        agentResponse += `Analyzing your Google Drive for: "${query}"\n\n`;

        try {
          // Search knowledge base for Drive content
          const queryEmbedding = await generateEmbedding(query);
          if (queryEmbedding) {
            const { data: driveData } = await supabase.rpc('match_knowledge_base', {
            ...
            if (driveData && driveData.length > 0) {
              const driveFiles = driveData.filter((item: any) =>
                item.source_type === 'google_drive' || item.category === 'file'
              );

              if (driveFiles.length > 0) {
                agentResponse += `### Found ${driveFiles.length} relevant files:\n\n`;
                driveFiles.forEach((file: any, i: number) => {
                  agentResponse += `**${i + 1}. ${file.title}**\n`;
                  agentResponse += `   - Relevance: ${(file.similarity * 100).toFixed(0)}%\n`;
                  if (file.content) {
                    agentResponse += `   - ${file.content.substring(0, 200)}...\n`;
                  }
                  if (file.metadata?.webViewLink) {
                    agentResponse += `   - [Open in Drive](${file.metadata.webViewLink})\n`;
                  }
                  agentResponse += `\n`;
                });
              } else {
                agentResponse += `No files found in Drive matching your query.\n\n`;
                agentResponse += `Try:\n`;
                agentResponse += `- Using different keywords\n`;
                agentResponse += `- Checking if files are indexed in Drive\n`;
              }
            } else {
              agentResponse += `No results found. Drive indexing may be in progress.\n`;
            }
          }
        } catch (error: any) {
          agentResponse += `\n⚠️ Error searching Drive: ${error.message}\n`;
        }
        break;

      case 'audio-intelligence':
        // Search audio transcriptions
        agentResponse = `## 🎵 Audio Intelligence Results\n\n`;
        agentResponse += `Searching audio transcriptions for: "${query}"\n\n`;
        ...
        try {
          ...
          if (audioData && audioData.length > 0) {
            ...
            const relevantAudio = audioData.filter((audio: any) =>
              audio.transcription &&
              audio.transcription.toLowerCase().includes(query.toLowerCase())
            );

            if (relevantAudio.length > 0) {
              agentResponse += `### Found ${relevantAudio.length} relevant audio files:\n\n`;
              relevantAudio.forEach((audio: any, i: number) => {
                agentResponse += `**${i + 1}. ${audio.original_filename}**\n`;
                agentResponse += `   - Duration: ${audio.duration || 'Unknown'}\n`;
                agentResponse += `   - Date: ${new Date(audio.created_at).toLocaleDateString()}\n`;
                if (audio.speakers && audio.speakers.length > 0) {
                  agentResponse += `   - Speakers: ${audio.speakers.length}\n`;
                }
                if (audio.transcription) {
                  const snippet = audio.transcription.substring(0, 250).replace(/\n/g, ' ');
                  agentResponse += `   - Preview: "${snippet}..."\n`;
                }
                agentResponse += `\n`;
              });
            } else {
              agentResponse += `No audio files contain the text: "${query}"\n\n`;
              agentResponse += `You have ${audioData.length} audio files total. Try a broader search.\n`;
            }
          } else {
            agentResponse += `No audio transcriptions found.\n\n`;
            agentResponse += `Upload audio files to analyze and search them.\n`;
          }
        } catch (error: any) {
          agentResponse += `\n⚠️ Error searching audio: ${error.message}\n`;
        }
        break;

      case 'knowledge-graph':
        // Search knowledge base
        agentResponse = `## 🕸️ Knowledge Graph Insights\n\n`;
        agentResponse += `Searching knowledge base for: "${query}"\n\n`;

        try {
          const queryEmbedding = await generateEmbedding(query);
          if (queryEmbedding) {
            const { data: kbData } = await supabase.rpc('match_knowledge_base', {
              query_embedding: queryEmbedding,
              match_threshold: 0.4,
              match_count: 15
            });
            ...
            if (kbData && kbData.length > 0) {
              agentResponse += `### Found ${kbData.length} related knowledge items:\n\n`;
              ...
              for (const [category, items] of Object.entries(byCategory)) {
                ...
                items.forEach((item: any, i: number) => {
                  agentResponse += `**${i + 1}. ${item.title}**\n`;
                  agentResponse += `   - Relevance: ${(item.similarity * 100).toFixed(0)}%\n`;
                  agentResponse += `   - Source: ${item.source_type || 'Unknown'}\n`;
                  if (item.content) {
                    agentResponse += `   - ${item.content.substring(0, 180)}...\n`;
                  }
                  agentResponse += `\n`;
                });
              }
            } else {
              agentResponse += `No related knowledge found.\n\n`;
              agentResponse += `The knowledge base is building as you use KimbleAI.\n`;
            }
          }
        } catch (error: any) {
          agentResponse += `\n⚠️ Error searching knowledge: ${error.message}\n`;
        }
        break;

      case 'project-context':
        // Get project info
        agentResponse = `## 📊 Project Context\n\n`;
        agentResponse += `Searching projects for: "${query}"\n\n`;
        ...
        try {
          ...
          if (projects && projects.length > 0) {
            ...
            if (relevantProjects.length > 0) {
              agentResponse += `### Found ${relevantProjects.length} related projects:\n\n`;
              relevantProjects.forEach((project: any, i: number) => {
                agentResponse += `**${i + 1}. ${project.name}**\n`;
                if (project.description) {
                  agentResponse += `   - ${project.description}\n`;
                }
                agentResponse += `   - Created: ${new Date(project.created_at).toLocaleDateString()}\n`;
                if (project.metadata) {
                  agentResponse += `   - Status: ${project.metadata.status || 'Active'}\n`;
                }
                agentResponse += `\n`;
              });
            } else {
              agentResponse += `No projects match "${query}"\n\n`;
              agentResponse += `You have ${projects.length} total projects.\n`;
            }
          } else {
            agentResponse += `No projects found.\n\n`;
            agentResponse += `Create projects to organize your work.\n`;
          }
        } catch (error: any) {
          agentResponse += `\n⚠️ Error searching projects: ${error.message}\n`;
        }
        break;

      case 'cost-monitor':
        // Get cost data
        agentResponse = `## 💰 Cost Monitor Report\n\n`;
        ...
        try {
          ...
          if (costData && costData.length > 0) {
            const totalCost = costData.reduce((sum, call) => sum + (call.cost_usd || 0), 0);
            const totalCalls = costData.length;

            // Group by model
            const byModel: Record<string, { calls: number; cost: number }> = {};
            costData.forEach((call: any) => {
              if (!byModel[call.model]) {
                byModel[call.model] = { calls: 0, cost: 0 };
              }
              byModel[call.model].calls++;
              byModel[call.model].cost += call.cost_usd || 0;
            });

            agentResponse += `### Last 30 Days Summary\n\n`;
            agentResponse += `- **Total API Calls:** ${totalCalls.toLocaleString()}\n`;
            agentResponse += `- **Total Cost:** $${totalCost.toFixed(2)}\n`;
            agentResponse += `- **Average per call:** $${(totalCost / totalCalls).toFixed(4)}\n`;
            agentResponse += `- **Daily average:** $${(totalCost / 30).toFixed(2)}\n\n`;

            agentResponse += `### Cost Breakdown by Model\n\n`;
            const sortedModels = Object.entries(byModel).sort((a, b) => b[1].cost - a[1].cost);
            sortedModels.forEach(([model, stats]) => {
              agentResponse += `- **${model}**: ${stats.calls} calls, $${stats.cost.toFixed(2)}\n`;
            });

            agentResponse += `\n### Recent Activity\n\n`;
            costData.slice(0, 10).forEach((call: any) => {
              agentResponse += `- ${new Date(call.timestamp).toLocaleString()}: ${call.model} - $${call.cost_usd.toFixed(4)}\n`;
            });
          } else {
            agentResponse += `No cost data available for the last 30 days.\n\n`;
            agentResponse += `Cost tracking will begin as you use the system.\n`;
          }
        } catch (error: any) {
          agentResponse += `\n⚠️ Error retrieving cost data: ${error.message}\n`;
        }
        break;

      default:
        agentResponse = `## 🤖 Agent: ${agentId}\n\n`;
        agentResponse += `Agent "${agentId}" is recognized but not yet fully implemented.\n\n`;
        agentResponse += `Coming soon! This agent will provide specialized intelligence for this domain.\n`;
    }
    ...
    try {
      await supabase.from('messages').insert([
        ...
        },
        {
          ...
          created_at: new Date().toISOString(),
          metadata: { mode: 'agent', agent: agentId }
        }
      ]);
    } catch (error) {
      console.warn('[AgentMode] Failed to save to history:', error);
    }
    ...
    console.error('[AgentMode] Error:', error);
    return NextResponse.json({
