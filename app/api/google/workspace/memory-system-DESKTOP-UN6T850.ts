...
interface CompressedMemory {
  ...
  metadata: {
    type: 'conversation' | 'transcription' | 'knowledge' | 'analysis' | 'audio' | 'image' | 'document' | 'pdf' | 'text' | 'word';
    userId: string;
    tags: string[];
    importance: number;
    created: string;
...
export class WorkspaceMemorySystem {
  ...
  async vectorSearchEfficient(
    ...
    const results: Array<{ id: string; content: string; similarity: number; metadata: any }> = [];
